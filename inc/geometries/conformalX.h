...
struct FpsiX
{
            ...
            double* R_0, double* Z_0, double& f, double& fp ) 
    {
        ...
        //while( eps > 1e-10)
        {
            ...
            temp = end;
            dg::stepperRK17( fieldRZYRYZY, temp, end, y_vec[n*N-1], 2.*M_PI, steps); //changed here
            if( psi <0)
 ...
 */
template< class container>
struct XGrid3d : public dg::GridX3d
{
    typedef dg::CurvilinearCylindricalTag metric_category;
    typedef XGrid2d<container> perpendicular_grid;
     ...
     */
    XGrid3d( solovev::GeomParameters gp, double psi_0, double fx, double fy, unsigned n, unsigned Nx, unsigned Ny, unsigned Nz, dg::bc bcx, dg::bc bcy): 
    ...
    perpendicular_grid perp_grid() const { return XGrid2d<container>(*this);}
 ...
 */
template< class container>
struct XGrid2d : public dg::GridX2d
{
    typedef dg::CurvilinearCylindricalTag metric_category;
    XGrid2d( const solovev::GeomParameters gp, double psi_0, double fx, double fy, double y, unsigned n, unsigned Nx, unsigned Ny, dg::bc bcx, dg::bc bcy): 
        dg::GridX2d( 0, 1, 0.,2*M_PI, n, Nx, Ny, bcx, bcy)
    {
        ...
        XGrid3d<container> g( gp, psi_0, fx,fy, n,Nx,Ny,1,bcx,bcy);
        f_x_ = g.f_x();
        f_ = g.f(), r_=g.r(), z_=g.z(), xr_=g.xr(), xz_=g.xz(), yr_=g.yr(), yz_=g.yz();
        g_xx_=g.g_xx(), g_xy_=g.g_xy(), g_yy_=g.g_yy();
        vol2d_=g.perpVol();
    }
    XGrid2d( const XGrid3d<container>& g):
...
thrust::host_vector<double> pullback( BinaryOp f, const conformal::XGrid2d<container>& g)
{
...
///@cond
template<class container>
thrust::host_vector<double> pullback( double(f)(double,double), const conformal::XGrid2d<container>& g)
{
...
thrust::host_vector<double> pullback( TernaryOp f, const conformal::XGrid3d<container>& g)
{
    thrust::host_vector<double> vec( g.size());
...
///@cond
template<class container>
thrust::host_vector<double> pullback( double(f)(double,double,double), const conformal::XGrid3d<container>& g)
{
